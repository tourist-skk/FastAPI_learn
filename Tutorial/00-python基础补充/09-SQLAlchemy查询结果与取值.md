...
> **先读 [08-ORM对象](08-ORM对象.md) 第五节。** 那里用一条规则概括了本篇所有类型：
>
> **`execute()` 永远返回 `Result`；`Result` 里永远装着 `Row`；`Row` 永远是个元组 —— 变的只是元组的格子里装什么。**
> `scalars()` 就是"每行取第 0 格"，`mappings()` 就是"把 Row 当字典"。
>
> 建立这个心智模型之后再看本篇：**08 讲的是"为什么"，本篇是查阅用的参考手册**，列出每个类型有哪些方法、各自的限制，以及 `execute()` 内部是怎么选类型的。不必从头背，按需查。
